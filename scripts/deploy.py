...
"""Copy the built core, its .mra files and capture blobs to a MiSTer.

    python scripts/deploy.py --rbf-only                          # build/output_files/MS32_stp.rbf
    python scripts/deploy.py --rbf-only --capture tetrisp-title  # also debug/tetrisp-title/capture.bin
    python scripts/deploy.py --mra-only      # skip the bitstream
    python scripts/deploy.py --dry-run       # show what would be copied

Ported from the Seta core's scripts/deploy.py (itself from Psikyo's); the
build guard below is the reason it exists. Capture blobs
(scripts/build_capture_blob.py) go to REMOTE_CAPTURES for the OSD's
"Load capture" file browser.
...
grossly fails timing (the .sdc header says so), and a bitstream that failed
...
REMOTE_ARCADE = "/media/fat/_Arcade/_Jaleco MegaSystem 32"
REMOTE_CAPTURES = "/media/fat/games/MS32"
SUCCESS = "Full Compilation was successful"

# Sets that are built but cannot yet run are held back from the device (name
# the .mra stems). Empty until the first game runs on hardware.
HELD_BACK_SETS = ()
...
def main():
    ...
    ap.add_argument("--capture", action="append", default=[],
                    help="capture name under debug/ whose capture.bin goes to the device (repeatable)")
    ...
    if not a.rbf_only:
        ...
        if not mras:
            sys.exit("no .mra files in releases/ -- none are generated yet (Phase 2)")
        ...
        if skipped:
            print(f"\n  SKIPPED {len(skipped)} held-back .mra file(s); pass --all to copy them anyway.")
