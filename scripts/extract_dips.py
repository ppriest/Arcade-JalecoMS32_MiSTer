#!/usr/bin/env python3
"""Extract a game's DIP switches from ms32.cpp's INPUT_PORTS_START blocks.

Copied from Arcade-Seta_MiSTer/scripts/extract_dips.py; the driver path and the game list are MS32's.

    python scripts/extract_dips.py tetrisp
    python scripts/extract_dips.py --all           # every set build_mra.py generates
...
DEF_STR = {
    ...
    "Low": "Low", "High": "High", "Highest": "Highest",
    "Easier": "Easier", "Harder": "Harder", "Very_Easy": "Very Easy",
    "Region": "Region", "Japan": "Japan", "USA": "USA", "Asia": "Asia",
}
...
def parse_ports(body, all_blocks, missing, depth=0):
    ...
    def add(entry):
        # PORT_MODIFY redefines bits an included block already declared (every
        # ms32.cpp game re-declares SW2/SW3 over the base block's
        # PORT_DIPUNUSED): the later field replaces the earlier ones it overlaps.
        ports[cur] = [d for d in ports[cur] if not (d[1] & entry[1])]
        ports[cur].append(entry)
    ...
    for raw in body.split("\n"):
        ...
        if m:
            if m.group(1) not in all_blocks and m.group(1).startswith("mahjong_"):
                continue   # MAME's shared mahjong key matrix (outside ms32.cpp): keys, no switches
        ...
        if m:
            ...
            off = defval & mask
            on = (~defval) & mask
            add(("Service Mode", mask, off, {off: "Off", on: "On"}))
            last = None
            continue
        ...
        m = re.match(rf'PORT_DIPUNUSED(?:_DIPLOC)?\(\s*{_NUM}\s*,\s*{_NUM}',
                     line)
        if m:
            add(("(unused)", _num(m.group(1)), _num(m.group(2)), None))
            last = None
            continue
        ...
        if m:
            ...
            add(("Service Mode", mask, dflt, {dflt: "Off", (~dflt) & mask: "On"}))
            last = None
            continue
...
GROUP_A = ["tetrisp", "p47aces", "gametngk", "hayaosi2", "tp2m32", "hayaosi3", "bbbxing", "suchie2",
           "desertwr", "gratia", "kirarast", "wpksocv2"]
