...
The ROMs-plus-capture `.mra` files (`scripts/build_mra.py --with-capture <cap>`) download the set
into SDRAM through `ms32_sdram_top`, decrypting the tiles on the way in, then load the capture blob
on rom index 2. `scripts/mister_launch.py` launches one, `scripts/mister_screenshot.py` takes the
native 320×224 frame, and `scripts/hw_compare.py <cap> <png> --tol 0` diffs it against the model.
On `MS32_stp` build 10000005 (commit 802b3fc):

| capture | layers drawn | board vs model |
|---|---|---|
| `tetrisp-title` | TX, BG, sprites | 100.00%, 71,680 of 71,680 exact |
| `p47aces-f1800` | TX, BG, ROZ, sprites | 100.00% |
| `gametngk-f3000` | TX, BG, ROZ, sprites (full screen) | 100.00% |

That also exercises the DDR3 sprite frame buffer's burst protocol on hardware, which the DDRAM
model could not prove. `sim/capload_tb` (Verilator) loads the capture as the board does, with reset
held across the download; `+OLD=1` reproduces the pre-fix board frame pixel for pixel.

The ROZ cache reads port B only in the cycle it consumes the data (`dpram`'s read enable): the
fitted M10K does not give "old data" on a same-address read during a write ("No - Unsupported Mixed
Feed Through Setting" in the RAM table).
