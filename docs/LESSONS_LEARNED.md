...
`Arcade-Seta_MiSTer`** -- three completed MiSTer arcade cores built by the same author against the
same toolchain, hardware and framework. Nothing in it describes Jaleco MegaSystem 32 hardware. It
is the accumulated cost of finding each of these the hard way, and the entire reason it is here is
so that this core does not pay for them a fourth time.
...
- `[Fuuki]` -- Fuuki
- `[Seta]` -- Seta
- `[MS32]` -- **this core.** Append new findings with that mark, in the same shape the existing
  entries use: **the rule, the mechanism that made the wrong assumption plausible, and the evidence
  that settled it.** Never edit a Psikyo, Fuuki or Seta entry to say something this core found.
...
- Entries referencing another project's module names (`tilemap_line_engine`, `psikyo_top`,
  `fuuki_sdram_top`, `x1_001`, `spriteram_dbuf`, ...) still state a *general* rule; the module name
  is evidence, not scope. The file it names does not exist in this repository.
...
| --- | --- |
| Write the V70 CPU core | "CPU cores (TG68K.C, T80)" -- **by analogy only**, since nothing is vendored here. The entries on clock-enable ratios, held DTACK, and budgeting the CPU as the Fmax-limiting block apply directly; the TG68K-specific ones are a description of what owning a CPU costs |
| Wire anything to the SDRAM controller | "Memory transport: req/valid contracts, latency, byte order" -- all of it |
| Size a RAM or add one | "Quartus synthesis gotchas", the four entries on block RAM. This design is BRAM-bound before a line of video RTL exists (see ROADMAP, "On-chip RAM budget") |
