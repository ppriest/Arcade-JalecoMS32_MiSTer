...
| Then | `Arcade-Seta_MiSTer/rtl/memory/sdram/` — copied from that Fuuki tree untouched. |
| Here | Copied from the Seta tree **untouched**, with the surrounding stack (`sdram_phy.sv`, `sdram_arbiter.sv`, `sdram_download.sv`, `sdram_narrow_bridge.sv`) and `sim/common/sdram_chip_model_wide.sv`. Do not edit. |
