...
//  fd25dd4 (rtl/ymf271_synth.sv), GPL-3 by its author's statement to the project owner;
//  see rtl/sound/ymf271/PROVENANCE.md. Changes, each marked "[MS32]":
//    2026-09-13  a clock enable, ce, on every clocked block.
//    2026-09-13  spi_defs.vh is replaced by ymf271_ms32.vh, which defines the
//                one name used here, SDR_PCM_BASE (0: ms32_sdram_top adds
//                the region base).
//    2026-09-13  the sample fetch address takes bit 21 when pcm_25mb is set, as
//                the wave-memory read path already did: MS32's sample ROM is
//                4 MB.
//============================================================================
//============================================================================
...
(
	input                clk,
	input                ce,           // [MS32] clock enable for every register
	input                reset,
	...
	// The sample region is 2.5 MB rather than 2 MB, so the chip's external
	// address has to reach 22 bits instead of 21. True only on the single-board
	// sets that load a second sample ROM above the first (SET_HAS_25MB_PCM in
	// spi_defs.vh). It is a per-set signal and not a constant because the extra
	// half megabyte is SDR_SND01_BASE's: free on those boards, but the CARTRIDGE
	// sets' Z80 program on every other one. Widening this unconditionally would
	// let a stray sample pointer read that program as audio, and the failure
	// would be a glitch with no obvious cause.
	input                pcm_25mb,
...
`include "ymf271_tables.vh"
	...
	generate
		for (b = 0; b < 8; b = b + 1) begin : par_lane
			...
			// [MS32] ce on each statement, not around the block: with the block
			// gated Quartus stopped inferring these eight lanes as RAM
			always @(posedge clk) begin
				if (we && ce) mem[wa] <= wd;
				if (ce) q <= mem[par_ra_mux];
			end
	...
	always @(posedge clk) if (ce) begin   // [MS32] ce
		...
		S_IDLE: if (!tick_pend && (ext_req != ext_ack)) begin
			...
			sdr_addr <= SDR_PCM_BASE + {3'd0, ext_addr[21] & pcm_25mb, ext_addr[20:3], 3'b000};
			sdr_req  <= ~sdr_req;
			ext_ret  <= S_IDLE;
			state    <= S_EXT;
		end
		...
		S_FEA0: begin
			if (hit_a) state <= S_FEB0;
				...
				sdr_addr <= SDR_PCM_BASE + {3'd0, fetch_base[21] & pcm_25mb, fetch_base[20:3], 3'b000};   // [MS32] bit 21
				sdr_req  <= ~sdr_req;
				state    <= S_FEA1;
			end
		end
		...
		S_FEB0: begin
			if (!need_b || hit_b) state <= S_INTP;
				...
				sdr_addr <= SDR_PCM_BASE + {3'd0, fetch_base[21] & pcm_25mb, fetch_base[20:3], 3'b000} + 26'd8;   // [MS32] bit 21
				sdr_req  <= ~sdr_req;
				state    <= S_FEB1;
			end
		end
	...
	wire _unused = &{1'b0, w0, w1, w2, w3, pm_t1, pm_t2, mod_mul, in_sum,
	                 ...
	                 // region is 2 MB on almost every set, so [20:0] addresses
	                 // all of it -- both flash chips included, since chip 1 is
	                 // just bit 20. Bit 21 is used only where the region is
	                 // 2.5 MB (pcm_25mb); bit 22 mirrors rather than reaching
	                 // anything on any set.
	                 ext_addr[22]};
	/* verilator lint_on UNUSEDSIGNAL */
