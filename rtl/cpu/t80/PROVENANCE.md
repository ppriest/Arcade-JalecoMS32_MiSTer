...
**In this repository:** copied from the Fuuki core's `rtl/cpu/t80/` (itself Psikyo's copy),
unchanged (every `.vhd` and the README; not `T80.qip`, since `files.qip` lists the six files the
build uses). MS32's sound CPU
is a Z80 at 8 MHz, driven through `T80se` with a clock enable by `rtl/sound/ms32_sound.sv`. The
notes below are Psikyo's, carried with the files.
